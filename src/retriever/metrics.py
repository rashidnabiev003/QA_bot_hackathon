...
class MetricComputer:
	def __init__(self, cfg: MetricConfig):
		...
		self.bleurt = None
		target_device = 'cpu'
		self.sas = FlagReranker(
			cfg.sas_model, use_fp16=cfg.sas_fp16, device=target_device
		)

	def _bleurt20_http(self, pred: str, ref: str) -> float:
		try:
			payload = json.dumps(
				{'references': [ref], 'candidates': [pred]}
			).encode('utf-8')
	...
	def bleurt20(self, pred: str, ref: str) -> float:
		...
		if self.bleurt is not None:
			try:
				return float(
					self.bleurt.score(references=[ref], candidates=[pred])[0]
				)
			except Exception:
				return 0.0
		return 0.0
	...
	def retrieval_metrics_semantic(
		self,
		gold_ctx: str | list[str],
		...
		gold_list: list[str] = (
			[gold_ctx] if isinstance(gold_ctx, str) else list(gold_ctx)
		)
		sims: list[float] = []
		...
		if relevant_positions:
			precisions = [
				(j + 1) / pos for j, pos in enumerate(relevant_positions)
			]
	...
	def compute_all(
		...
		gold_context: str | list[str] | None = None,
	) -> dict[str, Any]:
		result = {
			'bleurt': 0.0,
			'sas': 0.0,
		...
		if gold_context and contexts:
			...
			retr = self.retrieval_metrics_semantic(
				gold_context, contexts, k=k, rel_threshold=0.75
			)
...
def evaluate_query(
	...
	if gold_context is not None:
		out['retrieval'] = mc.retrieval_metrics_semantic(
			gold_context, texts, k=10
		)
	return out
