...
class QAPipeline:
	def __init__(self, index_dir: str | None = None):
		...
		logger.info(
			...
			self.index_dir,
			self.build_config.batch_size,
			self.build_config.chunk_size,
			self.build_config.overlap_size,
		)
		logger.info(
			'Metrics config: sas_model=%s, bleurt_endpoint=%s',
			self.metric_config.sas_model,
			self.metric_config.bleurt_endpoint,
		)

	def _ensure_index_loaded(self) -> None:
		try:
			# Опция принудительной пересборки индекса при старте
			force_rebuild = os.getenv('FORCE_REBUILD_INDEX', '0') in (
				'1',
				'true',
				'True',
			)
			if force_rebuild:
				logger.info(
					'FORCE_REBUILD_INDEX=1 — rebuilding index on startup'
				)
				self.rebuild_index()
				return
			...
			if not getattr(self.engine, 'ready', False):
				auto = os.getenv('AUTO_BUILD_INDEX', '0') in (
					'1',
					'true',
					'True',
				)
	...
	def _load_gold_triplets(self) -> None:
		...
		try:
			xlsx_path = os.getenv(
				'GOLD_XLSX_PATH',
				str(Path('src/data/base_triplets.xlsx').resolve()),
			)
			p = Path(xlsx_path)
			if not p.exists():
			...
			if 'question' not in df.columns or 'answer' not in df.columns:
				logger.warning(
					'Gold XLSX missing required columns: question, answer'
				)
				return
			cnt = 0
	...
	def build_index_from_docx(self) -> None:
		docx_path = os.getenv(
			'DOCX_PATH', str(Path('src/data/input.docx').resolve())
		)
		p = Path(docx_path)
		if not p.exists():
		...
		try:
			logger.info(
				'Building index from DOCX via LangChain loader: %s', docx_path
			)
			...
			text = '\n\n'.join(
				d.page_content for d in docs if d and d.page_content
			)
			if not text.strip():
	...
	async def answer(
		...
		logger.info(
			'Answer pipeline: topn=%s, topk=%s, llm_rerank=%s',
			topn,
			topk,
			use_llm_rerank,
		)
		...
		if use_llm_rerank and contexts:
			logger.info(
				f'Applying LLM reranking on {len(contexts)} contexts to select top {topk}'
			)
			t1 = time.monotonic()
			...
			logger.info(
				f'After LLM reranking: {len(contexts)} contexts, first scores: {[c.get("llm_score", 0) for c in contexts[:3]]}'
			)
		else:
		...
		logger.info(
			'Answer pipeline total time: %.3fs', time.monotonic() - t_start
		)
		return out
	...
	async def benchmark_from_xlsx(
		...
		for idx, row in df.iterrows():
			...
			logger.info(
				f'Benchmark: processing question {idx + 1}/{len(df)}: {question[:50]}...'
			)
			...
			try:
				...
				logger.error(
					f'Failed to compute metrics for question {qid}: {e}'
				)
				metrics_result = {
					'bleurt': 0,
					'sas': 0,
			...
			for i, ctx in enumerate(contexts):
				...
				if use_llm_rerank and 'llm_score' in ctx:
					rec[f'context_{i + 1}_llm_score'] = ctx.get(
						'llm_score', 0.0
					)
				elif 'score_rerank' in ctx:
					rec[f'context_{i + 1}_score_rerank'] = ctx.get(
						'score_rerank', 0.0
					)
				elif 'score' in ctx:
		...
		logger.info(
			f'Benchmark completed: {len(out_df)} rows saved to {out_path}'
		)
		return {'ok': True, 'path': out_path, 'rows': len(out_df)}


pipeline_singleton: QAPipeline | None = None
