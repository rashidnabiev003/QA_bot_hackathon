...
class SemanticSearch:
	...
	def __init__(
		self, index_dir: str | None = None, build_config: Any | None = None
	) -> None:
		self.index_dir: Path = Path(
			index_dir or os.getenv('INDEX_DIR', 'src/data')
		).resolve()
		...
		self.chunk_size: int = int(
			getattr(build_config, 'chunk_size', os.getenv('CHUNK_SIZE', 128))
		)
		self.chunk_overlap: int = int(
			getattr(
				build_config, 'overlap_size', os.getenv('CHUNK_OVERLAP', 50)
			)
		)
	...
	def _split_text(self, text: str) -> list[Document]:
		if self._tokenizer is None:
			self._tokenizer = AutoTokenizer.from_pretrained(
				self.embedding_model, use_fast=True
			)
	...
	def load(self) -> None:
		...
		try:
			...
			self._faiss = FAISS.load_local(
				str(self.faiss_dir), emb, allow_dangerous_deserialization=True
			)
			self.ready = True
			logger.info(
				{'event': 'index_loaded', 'faiss_dir': str(self.faiss_dir)}
			)
		except Exception as e:
	...
	def _make_retriever(
		self, top_k_dense: int, top_k_final: int
	) -> ContextualCompressionRetriever:
		if not self._faiss:
			raise RuntimeError(
				'FAISS is not loaded. Call load() or build_from_text() first.'
			)
		dense = self._faiss.as_retriever(search_kwargs={'k': top_k_dense})
		bm25 = _load_bm25_from_jsonl(self.bm25_jsonl, k=top_k_dense)
		hybrid = EnsembleRetriever(
			retrievers=[dense, bm25], weights=[0.7, 0.3]
		)
		device = (
			'cuda'
			if (self.emb_on_cuda and torch.cuda.is_available())
			else 'cpu'
		)
		ce = HuggingFaceCrossEncoder(
			model_name=self.rerank_model, model_kwargs={'device': device}
		)
	...
	def retrieve(
		self, query: str, topn: int = 40, topk: int = 8
	) -> dict[str, Any]:
		if not self.ready:
			self.load()
		if not self.ready:
